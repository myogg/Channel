...
BroadcastChannel hooks include `body.feed`, `ol.posts-feed`, `.post-entry`, `.post-meta`, `.post-content`, `.post-reactions`, `.reaction-paid`, `p.tags.post-tags`, `.post-comments`, and Telegram widget selectors. `PostEntry` also exposes `.hn-story`, `.post-entry-avatar`, `.post-entry-author`, and `.tg-message-meta` for narrowly scoped theme treatments. Post detail adds the read-aloud bar: `.post-audio` with `.post-audio-row`, `.post-audio-toggle`, `.post-audio-icon`, `.post-audio-label`, `.post-audio-divider`, `.post-audio-time`, `.post-audio-share`, and `.post-audio-status`. It sits directly under `.post-meta`, above `.post-content`, and only renders when the post has readable text. Its circular toggle is a deliberate exception to Base's square geometry, like avatars and back-to-top. Base hides all four by default in `feed.css`; overrides may reveal them without changing Base presentation. `.hn-story` wraps the real post title in an `h2` whenever one exists, while the avatar and author receive the real channel avatar and title from `PostsPage`; `.tg-message-meta` provides a message-style permalink time. `.tgme_widget_message_forwarded_from` is the stable forwarded-source hook: Base presents it as restrained supporting text without a card, background, or decoration, and optional themes may override that treatment.
...
Navigation, responsive search, expandable content, spoilers, image popovers, pagination, and back-to-top behavior use HTML and CSS rather than application JavaScript. Post-detail read-aloud and Telegram comments are the only two intentional browser-script exceptions; every other surface must stay script-free. The read-aloud bar renders its label, estimated duration, and share control server-side, so the script only adds playback and clipboard behavior.
